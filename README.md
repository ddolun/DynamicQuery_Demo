...
┌──────▼──────────┐      ┌─────────▼──────────┐
│ 前端展示層       │      │  後端共用層         │
│ DynamicQueryBox │      │  Query Builder      │
│ 讀取 Metadata    │ ───► │  接收 Query Object │
│ 動態渲染表單     │      │  產出 Parameterized│
│                 │      │  SQL 並執行         │
└─────────────────┘      └─────────────────────┘
```
...
若僅依序串接查詢條件，會因 AND / OR 的運算優先順序不同，導致實際查詢結果與使用者預期不符。
...
- ⏱️ **新增查詢頁時間**：2 工作天 → **2 小時**（約節省 90% 開發時間）
- 🔁 **重複程式碼**：每頁獨立開發 → **單一共用 Component**
- 🎨 **使用者體驗**：各頁面查詢風格不一 → **全系統統一查詢體驗**
- 🔒 **安全性**：固定查詢條件 → **動態查詢同時保留四層防禦**
- 🧩 **延展性**：使用者可自行組合欄位、運算子、排序，無需工程師介入
